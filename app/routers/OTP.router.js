...
module.exports = function (app) {
    ...
    app.post('/email/otpforget',[verifySignup.checkExistEmail], async (req, res) => {
        res.header('Access-Control-Allow-Credentials', true)
        const email = req.body.email;
        if (!email) {
            return res.status(400).send({message: 'Email not provided!'});
        ...
        try {
            sendMail(email, subject_mail, `<div>OTP for your email verification is: <span style='color: #6b7280; font-weight: bold'>${otp}</span></div>`);
            res.cookie('otpforget',otp, {expires: expiration_time, httpOnly: false}).send({message: "Mã OTP đã gửi vào email bạn!"});
        } catch (err) {
            console.log(err);
            res.status(500).send({message: err})
        }
